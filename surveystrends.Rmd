...
# The third would give the number of counts of a species over time, which could tell us when the animals were in the area
# Load lib
library(tidyverse)
library(directlabels)
...
yearly_counts <- surveys_complete %>%
  group_by(counts, species_id) %>%
  summarize(time = mean(time))

# Render a plotting counts versus time

ggplot(data = yearly_counts, aes(x = counts, y = yearly_counts, color = species_id)) +
  geom_line(size = 1.5, alpha = 0.5) +
  geom_dl(aes(label = species_id), method = list(dl.trans(x = x + 0.1), "last.points"), cex = 1.0) +
  theme_grey(base_family = "") +
  theme(axis.text.x = element_text(size = 11, angle = 0),
